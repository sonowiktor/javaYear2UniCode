...
public class WritingFunctions {
	...
	public static void main(String[] args) {
		...
		//part b)
		System.out.println("\nPart b)");
		Function<String, Character> fCh = s -> s.charAt(0);
		
		Stream.of("hello", "goodbye", "greetings", "bye").map(fCh).forEach(System.out::println);
	
	
		//part c)
		System.out.println("\nPart c)");
		
		ToIntFunction<Integer> sqr = x -> x*x;
		
		Stream.of(1,2,3,4,5).mapToInt(sqr).forEach(System.out::println);
		...
		Stream.of(1,2,3,4,5).mapToDouble(half).forEach(System.out::println);
